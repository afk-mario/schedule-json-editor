...
Spacebrew.Client = function(server, name, description, options) {
  ...
  if (window) {
    this.debug =
      window.getQueryString('debug') === 'true' ? true : options.debug || false;
  }
  this.reconnect = options.reconnect || true;
  this.reconnect_timer = undefined;

  this.sendRateCapped =
    options.capSendRate === undefined ? false : options.capSendRate;
  ...
  if (window) {
    this._name =
      window.getQueryString('name') !== ''
        ? unescape(window.getQueryString('name'))
        : this._name;
  }
  ...
  if (window) {
    this._description =
      window.getQueryString('description') !== ''
        ? unescape(window.getQueryString('description'))
        : this._description;
  }
...
Spacebrew.Client.prototype.send = function(name, type, value) {
  ...
  if (this.sendRateCapped) {
    ...
    if (!this.send_blocked) {
      ...
      setTimeout(function() {
        ...
        if (self.msg !== undefined) {
          // if message exists then sent it
          self.send(
            self.msg.message.name,
            self.msg.message.type,
    ...
    if (value instanceof ArrayBuffer) {
      ...
      var numBytesForJsonLength =
        jsonByteLength < 254 ? 1 : jsonByteLength <= 0xffff ? 3 : 5;
      var bufferSize =
        value.byteLength + jsonByteLength + numBytesForJsonLength;
      ...
      Spacebrew.encodeToBuffer(
        jsonString,
        new Uint8Array(newBuffer, numBytesForJsonLength, jsonByteLength)
      );
      var dataView = new Uint8Array(
        newBuffer,
        numBytesForJsonLength + jsonByteLength
      );
...
Spacebrew.Client.prototype._onOpen = function() {
  console.log(
    '[_onOpen:Spacebrew] Spacebrew connection opened, client name is: ' +
      this._name
  );
...
Spacebrew.Client.prototype._onMessage = function(e) {
  ...
  if (e.data instanceof ArrayBuffer) {
    ...
    if (jsonLength > 0) {
      try {
        var jsonString = Spacebrew.decodeFromBuffer(
          new Uint8Array(e.data, jsonStartIndex, jsonLength)
        );
        data = JSON.parse(jsonString);
        binaryData = {
          buffer: e.data,
          startIndex: jsonStartIndex + jsonLength,
        };
      } catch (err) {
        console.error(err);
        return;
  ...
  if (
    (!('targetType' in data) && !(data instanceof Array)) ||
    data['targetType'] == 'client'
  ) {
  ...
  if (this.reconnect && !this.reconnect_timer) {
    ...
    this.reconnect_timer = setInterval(function() {
      if (self.isConnected !== false) {
        self.connect();
        console.log(
          '[reconnect:Spacebrew] attempting to reconnect to spacebrew'
        );
      }
    }, 5000);
  }
...
Spacebrew.Client.prototype.name = function(newName) {
  if (newName) {
    // if a name has been passed in then update it
    if (this._isConnected) {
...
Spacebrew.Client.prototype.description = function(newDesc) {
  if (newDesc) {
    // if a description has been passed in then update it
    if (this._isConnected) {
